...
class Form {
    ...
    public function getQuestions($type = false, $qids = false){
        ...
        if($type){
            ...
            foreach($response->result as $line){ $qids[] = $line['question_id']; }
        }
    ...
    public function getSubmissions($sort = "created_at", $start = "0", $limit = "10", $dir = "ASC", $keyword=false, $onlyData = false, $qids = false, $noMarkup = false, $startDate = null, $endDate = null){
        ...
        foreach($response->result as $line){
            ...
            if($line["item_name"] != ""){
                ...
                if(!isset($items[$line['submission_id']])){ $items[$line['submission_id']] = array(); }
                if(!isset($items[$line['submission_id']][$line["question_id"]])){ $items[$line['submission_id']][$line["question_id"]] = array(); }
        ...
        foreach($response->result as $line){
            ...
            if(!isset($questions[$line["question_id"]])){ continue; }
            # If answers node of submissions array is empty then create a new one
            if(!isset($submissions[$line['submission_id']]["answers"])){ $submissions[$line['submission_id']]["answers"] = array(); }
    ...
    public static function getSubmissionResult($sid){
        ...
        foreach($response->result as $line){
            if(!$formID){ $formID = $line["form_id"]; } # get form ID from answers
            if($line["item_name"] != ""){
    ...
    public function getExtGridStructure($type){
        ...
        foreach($questions as $qid => $question){
            ...
            if(!self::isDataField($question["type"])){ continue; }
        ...
        foreach($submissions as $sid => $sub){
            if(is_array($sub["answers"])){
                ...
                foreach($sub["answers"] as $answer){
                    ...
                    if(!isset($answerRow[$answer["qid"]."_type"])){ $answerRow[$answer["qid"]."_type"] = $answer["type"]; }
                    
                    if(!empty($answer["items"])){ $answerRow[$answer["qid"]."_items"] = $answer["items"]; }
        ...
        if(isset($_GET["slug"])){
            ...
            switch($folder){
            	...
            	case "zip":
					...
					break;
                case "submit":
                    if($_GET){ $_GET['formID'] = $formname; }
                    if($_POST){ $_POST['formID'] = $formname; }
                	include ROOT."submit.php";
                    exit;
                	break;
                case "report":
                	if($_GET){ $_GET['reportID'] = $formname; }
                    if($_POST){ $_POST['reportID'] = $formname; }
                ...
                default:
                    ...
                    if(!$res->success){ Utils::errorPage("There is problem on the page, please correct your URL", "Error on the page"); }
    ...
    public function createDataDump($fieldList = array(), $filter = 'exclude', $startDate="", $endDate=""){
        ...
        foreach($questions as $qid => $question){ $sortedQuestions[$question['order']] = $question; }
        ksort($sortedQuestions);
        ...
        foreach($sortedQuestions as $sort => $question){
            if(in_array($question['type'], self::$nonDataFields)){ continue; }
            
            if($filter == 'exclude'){
                if(in_array($question['qid'], $fieldList)){ continue; }
            }else{
                if(!in_array($question['qid'], $fieldList)){ continue; }
            }

...
class UtilsRequests extends UtilsEmails{
    ...
    static function handleBase64Requests(){
        if(Utils::get('encoded') === false && Utils::get('enc') === false){ return; }     // If encoded was provided
    ...
    static function checkOnV2($missingPage){
        ...
        if(APP){return;} # Should not work for applications
    ...
    static function deleteCookie($name){
        ...
        if(DOMAIN){ setcookie($name, "", strtotime("-1 Year"), "/", ".".DOMAIN); }
    ...
    static function getLatestTweets($count = 4){
        ...
        foreach($tweets as $tweet){
            ...
            if($tweet['in_reply_to_user_id_str'] !== null){ continue; }
            # if this is a direct mention do not display here, sort of like a reply
            if(Utils::startsWith($tweet['text'], '@')){ continue; }
            # do not display our retweets
            if($tweet['retweeted'] === true){ continue; }
    ...
    static function getTweetStream(){
        ...
        if($tweets === false){ return ""; } # no tweet returned

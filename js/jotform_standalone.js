...
var JotForm = {
    ...
    getServerURL: function(){
        var form = JotForm.lib.select('.jotform-form')[0];
        if (form) {
    ...
    createConsole: function(){
        ...
        JotForm.lib.each(consoleFunc, function(c){
            this[c] = function(){
                if(JotForm.debug){
    ...
    init: function(callback){
        var ready = function(){
            try {
                ...
                JotForm.lib.each(document.forms, function(form){
    ...
    getDefaults: function(){
        JotForm.lib.each(JotForm.lib.select('.form-textbox, .form-dropdown, .form-textarea'), function(input){
    ...
    handleRadioButtons: function(){
        ...
        JotForm.lib.each(JotForm.lib.select('.form-radio-other-input'), function(inp){
            inp.disable().hint('Other');
        });
        
        JotForm.lib.each(JotForm.lib.select('.form-radio'), function(radio){
    ...
    handleAutoCompletes: function(){
        ...
        JotForm.lib.each($H(JotForm.autoCompletes), function(pair){
            var el = $(pair.key); // Field itself
            var parent = $(el.parentNode); // Parent of the field for list to be inserted
            var values = pair.value.split('|'); // Values for auto complete
            ...
            el.observe('keyup', function(e){
                ...
                // If matches found
                if (matches.length > 0) {
                    JotForm.lib.each(matches, function(match){
    ...
    prePopulations: function(){
        JotForm.lib.each($H(document.get), function(pair){
            var n = '[name*="_' + pair.key + '"]';
            var input = JotForm.lib.select('.form-textbox%s, .form-dropdown%s, .form-textarea%s'.replace(/\%s/gim, n))[0];
            if (input) {
                input.value = pair.value;
            }
            JotForm.lib.each(JotForm.lib.select('.form-checkbox%s, .form-radio%s'.replace(/\%s/gim, n)), function(input){
    ...
    editMode: function(){
        ...
        new Ajax.Request('server.php', {
            ...
            onComplete: function(t){
                ...
                if (res.success) {
                    JotForm.lib.each($H(res.result), function(pair){
                        ...
                        switch (question.type) {
                            ...
                            case "control_radio":
                                ...
                                JotForm.lib.each($A(radios), function(rad){
                            ...
                            case "control_checkbox":
                                var checks = JotForm.lib.select("#id_" + qid + ' input[type="checkbox"]');
                                
                                JotForm.lib.each($A(checks), function(chk){
                            ...
                            case "control_grading":
                                ...
                                JotForm.lib.each($A(boxes), function(box, i){
                            ...
                            case "control_matrix":
                                ...
                                JotForm.lib.each($A(question.items), function(item, i){
                                    if (Object.isString(item)) {
                                        var els = document.getElementsByName("q" + qid + "_" + question.name + "[" + i + "]");
                                        JotForm.lib.each($A(els), function(el){
                                            if (el.value == item) {
                                                el.checked = true;
                                            }
                                        });
                                    } else {
                                        JotForm.lib.each($A(item), function(it, j){
                                            var els = document.getElementsByName("q" + qid + "_" + question.name + "[" + i + "][]");
                            ...
                            case "control_fullname":
                                JotForm.lib.each($H(question.items), function(item){
                    ...
                    JotForm.lib.select('input[name="formID"]')[0].insert({
    ...
    hideField: function(field){
        ...
        JotForm.lib.each($('id_'+field).select('input, select, textarea'), function(input){
    ...
    getInputType: function(id){
        ...
        if($('input_'+id)){
            ...
            if(JotForm.lib.select('#id_'+id+' input')[0]){
                type = JotForm.lib.select('#id_'+id+' input')[0].readAttribute('type').toLowerCase();
            }
        }
    ...
    checkCondition: function(condition){
        ...
        JotForm.lib.each($A(condition.terms), function(term){
            try{
                switch(JotForm.getInputType(term.field)){
                    ...
                    case "radio":
                        ...
                        if (['isEmpty', 'isFilled'].include(term.operator)) {
                            var filled = JotForm.lib.select('#id_'+term.field+' input').collect(function(e){ return e.checked; }).any();
                        ...
                        JotForm.lib.each(JotForm.lib.select('#id_'+term.field+' input'), function(input){
        ...
        if(condition.type == 'field'){ // Field Condition
            ...
            if((condition.link.toLowerCase() == 'any' && any) || (condition.link.toLowerCase() == 'all' && all)){
                ...
                var sections = JotForm.lib.select('.form-section');
    ...
    setConditionEvents: function(){
        try {
            JotForm.lib.each($A(JotForm.conditions), function(condition){
            ...
            JotForm.lib.each($H(JotForm.fieldConditions), function(pair){
                var field = pair.key;
                ...
                $(field).observe(event, function(){
                    JotForm.log('Here');
                    JotForm.lib.each($A(conds), function(cond){
    ...
    countTotal: function(prices){
        ...
        var total = 0;
        JotForm.lib.each($H(prices), function(pair){
    ...
    highLightLines: function(){
        ...
        JotForm.lib.each(JotForm.lib.select('.form-line'), function(l, i){
            JotForm.lib.each(JotForm.lib.select('input, select, textarea, div, table div', l), function(i){
    ...
    getCollapseBar: function(element){
        ...
        if (element.hasClassName("form-section-closed") || element.hasClassName("form-section")) {
            return JotForm.lib.select('.form-collapse-table', element)[0];
        }
    ...
    enableButtons: function(){
        setTimeout(function(){
            JotForm.lib.each(JotForm.lib.select('.form-submit-button'), function(b){
                b.enable();
    ...
    setButtonActions: function(){
        ...
        JotForm.lib.each(JotForm.lib.select('.form-submit-reset'), function(b){
            b.onclick = function(){
        ...
        JotForm.lib.each(JotForm.lib.select('.form-submit-print'), function(print_button){
    ...
    initGradingInputs: function(){
        ...
        JotForm.lib.each(JotForm.lib.select('.form-grading-input'), function(item){
            item.observe('blur', function(){
                ...
                JotForm.lib.each(JotForm.lib.select(".form-grading-input", $(item.parentNode.parentNode)), function(sibling){
    ...
    handlePages: function(){
        var pages = [];
        JotForm.lib.each(JotForm.lib.select('.form-pagebreak'), function(page, i){
            ...
            JotForm.lib.select('.form-pagebreak-next', section).invoke('observe', 'click', function(){ // When next button is clicked
            ...
            JotForm.lib.select('.form-pagebreak-back', section).invoke('observe', 'click', function(){ // When back button is clicked
        ...
        if (pages.length > 0) {
            var allSections = JotForm.lib.select('.form-section');
            ...
            if (last) {
                ...
                var back = JotForm.lib.select('button', JotForm.lib.select('.form-pagebreak-back-container')[0])[0];
    ...
    handleFormCollapse: function(){
        var openBar = false;
        var openCount = 0;
        JotForm.lib.each(JotForm.lib.select('.form-collapse-table'), function(bar){
            ...
            if (section.className == "form-section-closed") {
                ...
                if (!(JotForm.lib.select('.form-collapse-hidden', section).length > 0)) {
                    openBar = section;
                    openCount++;
                }
            }
            bar.observe('click', function(){
                ...
                if (section.closed) {
                    ...
                    if (openBar && openBar != section && openCount <= 1) {
                        ...
                        JotForm.lib.each(JotForm.lib.select('.form-collapse-right-show', openBar), function(e){
                    ...
                    JotForm.lib.each(JotForm.lib.select('.form-collapse-right-hide', section), function(e){
    ...
    handlePayPalProMethods: function(){
        if ($('creditCardTable')) {
            JotForm.lib.each(JotForm.lib.select('.paymentTypeRadios'), function(radio){
                radio.observe('click', function(){
    ...
    validateAll: function(form){
        ...
        JotForm.lib.each(JotForm.lib.select('*[class*="validate"]'), function(input){
    ...
    errored: function(input, message){
        ...
        if (JotForm.isCollapsed(input)) {
            ...
            if (!collapse.errored) {
                JotForm.lib.select(".form-collapse-mid", collapse)[0].insert({
        ...
        //if(JotForm.debug){
            insertEl = JotForm.lib.select('.form-input', container)[0];
            if (!insertEl) {
                insertEl = JotForm.lib.select('.form-input-wide', container)[0];
            }
            if(!insertEl){
                insertEl = container;
            }
    ...
    corrected: function(input){
        ...
        if (JotForm.isCollapsed(input)) {
            var collapse = JotForm.getCollapseBar(input);
            if (collapse.errored) {
                var c= JotForm.lib.select(".form-collapse-mid", collapse)[0];
                JotForm.lib.setStyle(c, {color: ''})
                JotForm.lib.remove(JotForm.lib.select('img', c)[0]);
    ...
    hideButtonMessage: function(){
        JotForm.lib.select('.form-button-error').invoke('remove');
    },
    
    showButtonMessage: function(){
        this.hideButtonMessage();
        JotForm.lib.each(JotForm.lib.select('.form-submit-button'), function(button){
    ...
    validator: function(){
        ...
        JotForm.lib.each($A(JotForm.forms), function(form){ // for each JotForm form on the page 
                ...
                input.validateInput = function(){
                    ...
                    if (vals.include("required")) {
                        if (input.tagName == "INPUT" && (input.readAttribute('type') == "radio" || input.readAttribute('type') == "checkbox")) {
                            ...
                            try{
                                var test = JotForm.lib.select('*[name=*' + input.name.replace(/\[.*$/, '') + ']');
                                if (JotForm.lib.select('*[name=*' + input.name.replace(/\[.*$/, '') + ']').map(function(e){
            ...
            JotForm.lib.each(JotForm.lib.select('.form-upload'), function(upload){

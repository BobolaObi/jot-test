...
var Droppables = {
  ...
  show: function(point, element) {
    ...
    var drop, affected = [];
    /* JotForm fix for overflow:auto parents */
    var op = element.getOffsetParent();
    var opscroll = [0,0];
    if(op.nodeName !== 'BODY'){ opscroll = [op.scrollLeft, op.scrollTop]; }
  ...
  initDrag: function(event) {
    ...
    if(Event.isLeftClick(event) || event.touches) {
      ...
      if(src.preventInitDrag === true){
          return; // In order to fix stupid IE bug for JotForm
          // IE 7 inits drag event before the click and prevent click to be run
      }
      var pointer = [Event.pointerX(event), Event.pointerY(event)];
...
var SortableObserver = Class.create({
  ...
  onStart: function() {
    if(document._onedit){ // JotForm editable fix
...
var Sortable = {
  ...
  onHover: function(element, dropon, overlap) {
    ...
    if(overlap > .33 && overlap < .66 && Sortable.options(dropon).tree) {
      ...
      if(dropon.previousSibling != element) {
		...
		dropon.parentNode.insertBefore(element, dropon); //// This is where switch occurs
        ...
        element.updateOff(1); // Jotform dirty hack
      }
    } else {
      ...
      if(nextElement != element) {
        ...
        dropon.parentNode.insertBefore(element, nextElement); ///// This is where switch occures
	    ...
	    if(dropon.parentNode!=oldParentNode) 
           Sortable.options(oldParentNode).onChange(element);
        Sortable.options(dropon.parentNode).onChange(element);
        element.updateOff(2); // Jotform dirty hack
      }
    }
  },
  ...
  onEmptyHover: function(element, dropon, overlap) {
    ...
    if(!Element.isParent(dropon, element) && Element.empty(dropon)) {
      ...
      element.updateOff(3);  // Jotform dirty hack
    }
  },
  /* Patched by Tankut Koray (tankut@gmail.com) {*/        
  createGuide : function (element) {
    if(!Sortable._guide) {
        ...
        //Sortable._guide.style.width = element.style.width;
        //Sortable._guide.style.height = element.style.height;
        //Sortable._guide.className = element.className;

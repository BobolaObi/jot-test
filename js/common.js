var CommonClass ={
    ...
    initialize:function(){
        ...
        try{ // try for v8
    	    ...
    	    if(this.lang != "en" && this.lang != "tr"){
        		...
        		$$('.info').each( function(elem){ elem.hide(); });
    	    }
	...
	*/
            ...
            this.updateOnResize();
            this.alignGlow();
            setInterval(function(){ $this.alignGlow();  }, 900);
    ...
    themeSelector: function(){
        if($('themes-selector')){
            ...
            setTimeout(function(){ sc() }, 2000);
            
            ts.observe('click', function(){
                Utils.loadCSS('css/wizards/theme-selector.css');
                Utils.loadScript('js/wizards/theme-selector.js', function(){
    ...
    checkSubmissionsEnabled: function(callback){
        if(document.disableSubmissions){
            Utils.alert('JotForm is currently under a maintenance mode. Our first priority is to keep your forms working. That\'s why Submissions page and Reports are <b>temporarily unavailable today between 9am-5pm EST</b>. Please check back later. We are sorry for the inconvenience.', 'Temporarily Unavailable', false, {
                noCenter:true,
                width:400
            });
            return false;
    ...
    setLoadingIndicator: function(){
        ...
        var $this = this;
        var hide = function(text){ /*$(document.body).removeClassName('waiting');*/ $('loading-indicator').shift({bottom:-40, duration:0.5}); };
        var show = function(text){ /*$(document.body).addClassName('waiting');*/    $('loading-indicator').shift({bottom:0, duration:0.5, link:'ignore'}); };
        
        $('loading-indicator').observe('click', function(){ hide(); });
    ...
    updateOnResize: function(){
        var resizeUpdater = function(e){
            ...
            if(width < 950){
                ...
                ($('themes-selector') && $('themes-selector').hide());
            }else{
                ($('feedback-tab') && $('feedback-tab').show());
                ($('themes-selector') && $('themes-selector').show());
            }
    ...
    Request: function(options){
        ...
        options = Object.extend({
            ...
            asynchronous: true,
            parameters: {},
            server: this.HTTP_URL+"server.php",
            evalJSON: "force",
            method: 'post'
        }, options || {});
        ...
        new Ajax.Request(options.server, {
            ...
            onComplete: function(t){
                try {
                    if(t.status === 0){ return; } // aborted
                    
                    var res = {success: false, error:'Cannot evaluate JSON: '+ t.responseText};
    ...
    scrollLimits: function(element, top, btop){
        ...
        if(this.isFullScreen /*|| Prototype.Browser.IE9*/){ return; }
    ...
    screenToggle: function (button){
        if(!this.isFullScreen){
            ...
            ($('search-bar') && $('search-bar').setStyle({left:'201px', width:''}));
    ...
    /**
     * Calculates the max scrool for window
     * @param {Object} pass
     */
    ...
    updateBarHeightInFullScreen: function (){
        ...
        ($('search-bar') && $('search-bar').setStyle({left:'200px', width:(dwidth-216)+'px'}));
        ($('prop-tabs') && $('prop-tabs').setStyle({width: (document.viewport.getWidth()-200)+'px'}));
    ...
    setToolboxFloat: function(){
        window.scrollTo(0, 0);
        $('tools-wrapper')[/*Prototype.Browser.IE9? 'keepInViewport' : */'positionFixed']({offset:69, onBeforeScroll: this.scrollLimits, onBeforeScrollFail: this.fixBars});
    },
    /**
    ...
    alert: function (mess, title, callback, options){
        ...
        this.messageWindow = document.window({
            ...
            onInsert: function(w){
                ...
                setTimeout(function(){ w.buttons.OK.focus(); }, 100);
                options.onInsert(w);
            }
        });
        ...
        this.messageWindow = document.window({
            ...
            onInsert: function(w){
                ...
                input.observe('keydown', function(e){
                    e = document.getEvent(e);
                    if(e.keyCode == 13){ w.buttons.OK.run('click'); }
                });
            }
        });
    ...
    confirm: function (mess, title, callback){
        ...
        this.messageWindow = document.window({
            ...
            onInsert: function(w){
                w.buttons.OK.setStyle({fontWeight:'bold'});
                setTimeout(function(){ w.buttons.Cancel.focus(); }, 100);
            }
        });
        return this.messageWindow;
    },
    ...
    poof: function (e){
        ...
        setTimeout(function(){
            interval = setInterval(function(){
                if(c >= 4){ clearInterval(interval); img.remove(); return true;  }else{ c++;}
    ...
    formatPrice: function (amount, curr, id, nofree){
        ...
        switch(curr){
            ...
            case "EUR":	return "&euro;<span id=\""+id+"\">"+amount + ' EUR';
            case "GBP":	return "&pound;<span id=\""+id+"\">"+amount + ' GBP';
    ...
    setAccordion: function (accordionContainer, options){
        var openPanel = undefined; // Currently open panel
        // get all panels
    ...
    loadTemplate: function (template, callback){
        try{
            if(this.templateCache[template]){
                ...
                new Ajax.Request(template+"?"+this.session, {
                    method:'get',
                    onComplete: function(t){
                        try{
                        if (t.status != 200) {
                            Utils.alert('There is an error on the wizard. Please contact Jotform Support.'.locale());
                            return;
                        }
    ...
    loadScript: function (file, callback, arg){
        ...
        if(this.templateCache[file]){
            ...
            try{
                this.templateCache[file] = callback || function(){ };
                var script = document.createElement('script');
                script.type="text/javascript";
    ...
    loadCSS: function (url){
        if($A(this.loadedLinks).include(url)){ return; }
    ...
    tryCSSLoad: function (callback){
        if(this.cssloaded){ this.cssloaded = false; return; }
        try{ 
            callback(); 
            this.cssloaded = true;
        }catch(e){
    ...
    checkEmailFormat: function(email) {
        if(!Object.isString(email)){ return false; }
    ...
    redirect: function(url, options){
        ...
        /*
         * Causing problems on redirect without parameters
         * and using _blank option.
         * 
    ...
    defaultEmail: function(type, textOnly){
        ...
        def += '<img src="'+Utils.HTTP_URL+'images/logo-small.png" align="right" border="0" />';
        def += '</a>';
        def += '</div>';
        ...
        if (type == 'autorespond') {
            ...
            if(form.getProperty('id')){
                ...
                def += 'View this submission on JotForm'.locale();
                def += '</a>';
            }else{

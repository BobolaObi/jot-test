...
{
    global $_MPM;
    $_MPM['Serial'][] = "bcadd($zahl1,$zahl2)";

    if ($zahl1 === false or $zahl2 === false) {
        return false;
    }
    ...
    while (substr($zahl1, 0, 1) == "0") {
        $zahl1 = substr($zahl1, 1, strlen($zahl1) + 1);
    }
    while (substr($zahl2, 0, 1) == "0") {
        $zahl2 = substr($zahl2, 1, strlen($zahl2) + 1);
    }

    # delete positive sign
    while (substr($zahl1, 0, 1) == "+") {
        $zahl1 = substr($zahl1, 1, strlen($zahl1) + 1);
    }
    while (substr($zahl2, 0, 1) == "+") {
        $zahl2 = substr($zahl2, 1, strlen($zahl2) + 1);
    }
    ...
    # add leading zeros
    if (strlen($teil_vor_komma1) < strlen($teil_vor_komma2)) {
        $zahl1 = str_repeat("0", strlen($teil_vor_komma2) - strlen($teil_vor_komma1)) . $zahl1;
    }
    if (strlen($teil_vor_komma1) > strlen($teil_vor_komma2)) {
        $zahl2 = str_repeat("0", strlen($teil_vor_komma1) - strlen($teil_vor_komma2)) . $zahl2;
    }
    ...
    for ($i = strlen($zahl1) - 1; $i >= 0; $i--) {
        ...
        $er = $a + $b;
        if ($er > 9 and $i != 0) {
            $ueberlauf = substr($er, 0, strlen($er) - 1);
            $er = substr($er, -1);
        } else {
            $ueberlauf = 0;
        }
    ...
    if ($pos) {
        ...
        # delete trailing zeros
        while (substr($fg, strlen($fg) - 1, strlen($fg)) == "0") {
            $fg = substr($fg, 0, strlen($fg) - 1);
        }
        # delete trailing decimal point if it is the last sign
        if (substr($fg, strlen($fg) - 1, strlen($fg)) == ".") {
            $fg = substr($fg, 0, strlen($fg) - 1);
        }
    }
...
function bcsub($zahl1, $zahl2)
{
    ...
    if ($teil_vor_komma1 < $teil_vor_komma2) {
        $help = $zahl2;
        $zahl2 = $zahl1;
        $zahl1 = $help;
        $vorzeichen = "-";
    }
    if (($teil_vor_komma1 == $teil_vor_komma2) and ($teil_hinter_komma1 < $teil_hinter_komma2)) {
        $help = $zahl2;
        $zahl2 = $zahl1;
        $zahl1 = $help;
        $vorzeichen = "-";

 ...
 */class LDAPInterface{
    ...
    function normalizeSearchResult($result){
        ...
        foreach($result as $key => $value){
            if($key == "count") { continue; } # We don't need count key. Obviously, we can count this stupid array
            if(is_numeric($key)){ continue; } # This should be an associative array no need for array indexes
            if(is_array($value)){ $value = $value[0]; } # usually the first argument is what we needed

<?php
        ...
        // tayfunsen Since the executable is in the path, there's no need for abs. path.
        // $GLOBALS['WKPDF_BASE_PATH']=str_replace(str_replace('\\','/',getcwd().'/'),'',dirname(str_replace('\\','/',__FILE__))).'/';
        $GLOBALS['WKPDF_BASE_PATH'] = '/usr/sbin/';
        ...
        class WKPDF {
                ...
                private static $cpu='';
                private $url = '';
                // This is the temporary file which has the HTML code to be exported to PDF.
                private $tmp = '';
                /**
                ...
                private static function _getCPU(){
                        if(self::$cpu==''){
                                // Patched by tayfunsen to work on iMac.
                                if (shell_exec('uname -a | grep -i amd -') != '') {
                                    self::$cpu='amd64';
                                } else if (shell_exec('uname -a | grep - \'i.86\' -') != '') {
                                    self::$cpu='i386';  
                                } else {
                 ...
                 */
                public function set_html($html, $tmp_path){
                        $this->set_tmp_path($tmp_path);
                        $this->html=$html;
                ...
                public function get_help(){
                        $help_output = self::_pipeExec('"'.$this->cmd.'" --extended-help');
                ...
                public function render(){
                        // Check if a URL or HTML source was given.
                        if ($this->url != '') {
                            $website = $this->url;                             
                        } else if ($this->tmp != '') {
                            $website = $this->tmp;
                        ...
                        $this->pdf=self::_pipeExec(
                                ...
                                . ' --enable-forms -n ' // enable forms to be filled in and do not run JS.
                                .' "' . $website . '" -'                          // URL and optional to write to STDOUT
                        );
                        // tayfunsen
                        // Console::error($this, "WKPDF ERROR **** ");

...
class Submission
{
    ...
    public static function continueSubmission($id, $type = '', $continuous = false, $additionalInfo = false)
    {
        ...
        if ($response->rows > 0) {
            ...
            if ($submit === false) {
                throw new \Exception("Cannot unserialize pendining submission");
            }
    ...
    private function debug()
    {
        ...
        Utils::print_r($this->request);
        if ($this->formHasPayment()) {
            echo "<h3>Form has a payment</h3>";
            echo "<hr><br> Payment text:";
            echo $this->makeProductText($this->questions[$this->paymentField['qid']]);
            echo "<hr><br> Payment Field:<br>";
            Utils::print_r($this->paymentField);
            echo "<hr><br> Products To Buy:<br>";
            Utils::print_r($this->productsToBuy);
            echo "<hr><br> Products:<br>";
            Utils::print_r($this->form->getProducts());
        }
        echo "<hr><br> Questions:<br>";
        Utils::print_r($this->questions);
        echo "<hr><br> Question Names:<br>";
        Utils::print_r($this->questionNames);
        if ($this->formHasUpload()) {
            echo "<h3>Form has upload</h3>";
            echo "<hr><br> Files:<br>";
            Utils::print_r($this->files);
            echo "<hr><br> Uploads:<br>";
        ...
        echo "<hr><br> Old Email Generated:<br>";
        Utils::print_r($emails->createOldEmail());
        echo "<hr><br> Form:<br>";
        Utils::print_r($this->form);
        echo "<hr><br> Form Questions:<br>";
        Utils::print_r($this->formQuestions);
        echo "<hr><br> Owner:<br>";
        Utils::print_r($this->owner);
        echo "<hr><br> Conditions:<br>";
        Utils::print_r($this->getConditions());
        unset($this->formProperties["form_emails"]);
        echo "<hr><br> Form Properties:<br>";
        Utils::print_r($this->formProperties);
        exit;
    }
    ...
    public function makeProductText($selectedProducts)
    {
        ...
        $type = "";
        foreach ($selectedProducts as $product) {
            if (!is_array($product) || !isset($product['paymentType'])) {
                continue;
            }
    ...
    private function handleUploads()
    {
        ...
        if (!$this->formHasUpload() || empty($this->uploads)) {
            return;
        }
        ...
        if ($uploads = $this->form->getQuestions('control_fileupload')) {
            ...
            foreach ($this->uploads as $qid => $uploadProp) {
                ...
                foreach ($props as $uploadProp) {
                    ...
                    if (strlen($uploadProp["name"]) <= 3 || $uploadProp["size"] <= 0) {
                        continue;
                    }
